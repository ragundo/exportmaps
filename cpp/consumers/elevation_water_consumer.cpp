...
RGB_color RGB_from_elevation_water(RegionDetailsElevationWater& rdew, int x, int y, int biome_type, df::world_region* region);
        ...
        else // There's data to be processed
        {
          ...
          for (auto x=0; x<16; ++x)
            for (auto y=15; y>=0; --y)
            {
                ...
                ExportedMapDF* elevation_water_map = maps_exporter->get_elevation_water_map();
...
/*****************************************************************************
Utility function
Return the RGB values for the elevation repecting water levels export map
*****************************************************************************/
...
RGB_color RGB_from_elevation_water(RegionDetailsElevationWater& rdew, int x, int y, int biome_type, df::world_region* region)
{
    ...
    if (has_river)
    {
        ...
        if ((river_horiz_y_min         != -30000) && (rdew.get_rivers_horizontal().elevation[x][y] < 30000))
              elevation = rdew.get_rivers_horizontal().elevation[x][y];

        if ((river_horiz_y_min_plus_one_row != -30000) && (rdew.get_rivers_horizontal().elevation[x+1][y] < elevation))
              elevation = rdew.get_rivers_horizontal().elevation[x+1][y];

        if ((river_vert_x_min         != -30000) && (rdew.get_rivers_vertical().elevation[x][y] < elevation))
              elevation = rdew.get_rivers_vertical().elevation[x][y];

        if ((river_vert_x_min_plus_one_col != -30000) && (rdew.get_rivers_vertical().elevation[x][y+1] < elevation))
