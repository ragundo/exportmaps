...
int get_site_total_population(df::world_site* world_site)
{
    int v2 = world_site->unk_1.nemesis.size() + world_site->unk_1.units.size();
    int v4 = 0;

    if (world_site->unk_1.inhabitants.size() == 0)
        return v2;

    for (unsigned int i = 0; i < world_site->unk_1.inhabitants.size(); i++)
    {
        df::world_site_inhabitant* inhabitant = world_site->unk_1.inhabitants[i];
