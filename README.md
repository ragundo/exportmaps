...
## What do it does?
It exports all the world maps while ypu are playing the game (Fortress or Adventure). There's no need to enter Legends mode.
...
The world is visited only once, generating the data and the different threads process that data simultaneously to generate the maps.
...
* The exported map are generated in .PNG format instead of .BMP to reduce disk space.
...
If you want to generate only a specific map, you can provide the map type that you want unsing a command line option:
